...
const SESSION_STORAGE_KEY = 'radix_tribes_session';
...
export function logout(): void {
    ...
    try {
        const { getSocket } = await import('./config');
        const socket = getSocket();
        if (socket && socket.connected) socket.emit('logout');
    } catch {
        /* ignored – socket layer not available */
    }
}
...
export function getCurrentUser(): User | null {
    ...
    try {
        const data = JSON.parse(sessionJson) as { user: User; token: string; expiresAt: number };
...
function persistSession(user: User): void {
    ...
    // Legacy (in-memory) storage for older code
    sessionStorage.setItem('radix_user', JSON.stringify(user));
}

function clearPersistedSession(): void {
    localStorage.removeItem(SESSION_STORAGE_KEY);
    sessionStorage.removeItem('radix_user'); // legacy clean-up
}

// Refresh expiry (e.g., user performed an action)
export function refreshSession(): void {
    const data = getCurrentUser();
    if (data) persistSession(data);
}
