...
// Lazy singleton WebSocket
const socket = getSocket();

// Expose connection status for React components
export const getConnectionStatus = () => _getConnectionStatus();

/**
 * Perform a socket.io request-response call with automatic
 * timeout & graceful fallback to local storage.
 *
 * @param event      Socket.IO event name
 * @param payload    Data to emit (optional)
 * @param timeoutMs  How long to wait before giving up
 */
const socketCall = <RespType, PayloadType = any>(
    event: string,
    payload?: PayloadType,
    timeoutMs = 2000,
): Promise<RespType> =>
    new Promise<RespType>((resolve, reject) => {
        ...
        // Use acknowledgement pattern
        try {
            socket.emit(event, payload, (response: RespType) => {
                clearTimeout(timer);
                resolve(response);
            });
        } catch (err) {
...
export async function getGameState(): Promise<GameState> {
    ...
    if (socket.connected) {
        try {
            return await socketCall<GameState>('getGameState');
        } catch (err) {
            // fall through to local fallback
            console.warn('Remote getGameState failed – falling back to local:', err);
        }
    }

    // 2. Local fallback (single-player in browser)
    return apiCall(() => {
